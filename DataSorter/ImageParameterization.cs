...
namespace DataSorter
{
    class ImageParameterization
    {
        ...
        private int[] classOfTestImageNum;
        ...
        public bool Calculate_mN(int N)
        {
            ...
            if (histogramCalcFlag && (N > 1) && ((paramCounter) < paramNum))
            {
                for (int i = 0; i < testImageNum; i++)
                {
                    ...
                    if (N == 2) testImageResults[i][paramCounter] = Math.Sqrt(testImageResults[i][paramCounter]);
                }
                paramCounter++;
            }
            return histogramCalcFlag;
        ...
        public bool LoadImages(string folderName, int numberOfClass)
        {
            Bitmap image;
            string fName;
            try
            {
                {
                    ...
                    for (int classN = 0; classN < numberOfClass; classN++)
                    {
                        for (int i = 0; i < testImageNum; i++)
                        {
